...
export class Grid2<Cell> {
  ...
  static fromGen<C>(size: ConstIVec2, gen: (at: ConstIVec2) => C): Grid2<C> {
    return new Grid2(size, Array.from(Array(size[0] * size[1]))).map((_, at) => gen(at));
  }
...
export function mathStateTick(state: MathState): MathState {
  ...
  const players = state.players.map((player) => {
    ...
    Array.from(Array(grid.size[1] - 1)).forEach((_, y) => {
      Array.from(Array(grid.size[0])).forEach((_, x) => {
